...
Backend server untuk monitoring sistem dan lingkungan real-time menggunakan Golang, Echo framework, dan SQLite.

## Fitur Utama

- **Multi-Agent Monitoring**: Monitor multiple servers from a central location
- **Comprehensive System Metrics**: CPU, Memory, Disk, Network, Process, Load Average
- **Environmental Monitoring**: Temperature & Humidity dari sensor (stored in SQLite)
- **WebSocket Terminal**: Interactive shell access through web interface
...
- **RESTful API**: Clean API untuk semua metrics dan agent management

## Teknologi yang Digunakan

- **Golang** v1.25+
- **Echo** v5 - Web framework
- **SQLite3** - Embedded database (no external database required!)
...
- Golang 1.25 atau lebih baru (with CGO enabled for SQLite)
- Git
- GCC compiler (untuk kompilasi SQLite driver)
...
**Windows:**
Install MinGW-w64 atau TDM-GCC
...
Edit file `.env` (optional, defaults work out of the box):
...
Migration akan dijalankan secara otomatis.

Tidak perlu setup eksternal database - semuanya self-contained.
...
Server akan:
1. Membuat direktori `./data` jika belum ada
2. Membuat database `monitoring.db`
3. Menjalankan migrations otomatis
4. Mulai listen di `http://localhost:8080`
...
```bash
# Basic usage
./agent -server http://main-server:8080 -name "Production Server 1"

# With all options
./agent \
  -server http://main-server:8080 \
  -name "Production API Server" \
  -description "Main API backend server" \
...
**Agent Flags:**
- `-server`: Main monitoring server URL (required)
- `-name`: Agent display name (required)
- `-description`: Optional description
- `-tags`: Comma-separated tags
- `-metrics-interval`: How often to send metrics (default: 30s)
- `-heartbeat-interval`: How often to send heartbeat (default: 60s)

The agent will:
1. Register itself with the main server
2. Get a unique ID from the server
3. Start collecting and sending system metrics
4. Send periodic heartbeats to maintain online status
